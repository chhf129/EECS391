...
import edu.cwru.sepia.agent.planner.GameState.TownHallInfo;
...
public class AstarAgent{
 ...
 /*   public AstarAgent(int playernum)
    ...
    public Map<Integer, Action> initialStep(State.StateView newstate, History.HistoryView statehistory) {
        ...
        if(!newstate.getUnit(footmanID).getTemplateView().getName().equals("Peasant"))
        {
            System.err.println("Peasant unit not found");
            return null;
        }
...
*/
     ...
     * @return
     */
    public Stack<MapLocation> findPath(GameState gs, int peasantID, Position end)
    {
        Peasant peas = null;
        ...
        MapLocation startLoc = new MapLocation(peas.location.x, peas.location.y, null, 0);

        MapLocation goalLoc = new MapLocation(end.x, end.y, null, 0);
        ...
        for (ResourceInfo ri: gs.goldmines){
        	resourceLocations.add(new MapLocation(ri.location.x, ri.location.y, null, 0));
        }
        for (ResourceInfo ri: gs.forests){
        	resourceLocations.add(new MapLocation(ri.location.x, ri.location.y, null, 0));
        }
        resourceLocations.add(new MapLocation(gs.townHall.location.x, gs.townHall.location.y, null, 0));

        return AstarSearch(startLoc, goalLoc, gs.xBound, gs.yBound, footmanLoc, resourceLocations);
    }
    /**
